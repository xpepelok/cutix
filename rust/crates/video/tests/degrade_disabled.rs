...
fn h264_still_decodes_with_ffmpeg_switched_off() {
    ...
    let info = match video::backend::probe(&path) {
        Ok(info) => info,
        Err(error) => panic!("probe must survive without ffmpeg: {error}"),
    };
    assert_eq!(u32::from(info.width), fixtures::SQUARE_CLIP_WIDTH);

    let frame = video::backend::frame_at(&path, 0.0).expect("decode must survive without ffmpeg");
...
fn webm_is_refused_rather_than_half_opened_without_ffmpeg() {
    ...
    assert!(
        matches!(
            video::backend::frame_at(&path, 0.0),
...
fn the_ffmpeg_backend_reports_why_it_cannot_serve() {
    ...
    disable_ffmpeg();
    let error = match video::backend::FfmpegBackend.probe(std::path::Path::new("anything.webm")) {

...
fn the_unified_stream_matches_the_one_shot_decode() {
    ...
    for seconds in [0.0, 0.5, 1.0] {
        ...
        let one_shot = video::backend::frame_at(&path, seconds).expect("one shot");
        assert_eq!(
...
fn hevc_in_mp4_falls_back_from_openh264_to_ffmpeg() {
    ...
    let frame = video::backend::frame_at(&path, 0.5)
        .expect("the seam must fall back to ffmpeg for HEVC in mp4");
    is_plausible_picture(&frame);

    let candidates = video::backend::backends_for(&path);
...
fn hevc_in_mp4_probes_to_its_coded_size_not_the_tkhd_display_size() {
    ...
    let frame = video::backend::frame_at(&path, 0.5).expect("decode");
    eprintln!(

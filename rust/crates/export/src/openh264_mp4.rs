...
use openh264::encoder::{
    BitRate, Encoder, EncoderConfig, FrameRate as EncoderFrameRate, FrameType, IntraFramePeriod,
    Level, Profile, RateControlMode, SpsPpsStrategy,
};
use openh264::formats::{RgbSliceU8, YUVBuffer};
use openh264::{OpenH264API, Timestamp};

use crate::aac::{self, AAC_FRAME_SAMPLES};
use crate::backend::{
...
fn audio_sample_duration(sample_rate: u32) -> Option<u32> {
    let numerator = TIMESCALE as u64 * AAC_FRAME_SAMPLES as u64;
...
impl OpenH264Mp4Backend {
    fn new(destination: &Path, spec: VideoSpec) -> Result<Self> {
        ...
        let config = EncoderConfig::new()
            .bitrate(BitRate::from_bps(spec.bitrate_bps))
...
impl EncoderBackend for OpenH264Mp4Backend {
    ...
    fn push_frame(&mut self, rgba: &[u8]) -> Result<()> {
        ...
        let index = self.frames + self.pending.len() as u64;
        let timestamp =
        ...
        if matches!(frame_type, FrameType::Skip | FrameType::Invalid) {
            return Err(ExportError::Encoder(format!(
                "encoder produced no data for frame {} ({frame_type:?})",
                self.frames
            )));
        }
...
fn patch_sl_config_predefined(path: &Path) -> Result<bool> {
...
fn split_annex_b(stream: &[u8]) -> (Option<(Vec<u8>, Vec<u8>)>, Vec<u8>) {
    let mut sps = None;
    let mut pps = None;

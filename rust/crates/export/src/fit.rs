...
pub fn blit_centre(
    ...
    let copy_width = source_width.min(target_width.saturating_sub(offset_x));
    let copy_height = source_height.min(target_height.saturating_sub(offset_y));
    for row in 0..copy_height as usize {
        let source_start = row * source_width as usize * 4;
        let target_start =
            ((row + offset_y as usize) * target_width as usize + offset_x as usize) * 4;
...
mod tests {
    ...
    fn the_bars_are_opaque_black_and_the_inner_pixels_survive() {
        ...
        blit_centre(&source, 2, 2, &mut target, 4, 4, 1, 1);

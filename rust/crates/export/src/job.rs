...
use cutix_playback::{mix, AudioCache, ComposeRequest, FrameComposer, MediaResolver, MixRequest};
use cutix_project::Project;
use time::{FrameRate, MediaTime};

use crate::backend::{AudioSpec, AudioSupport, BackendFactory, ExportArtifacts, VideoSpec};
    ...
    while written < total_frames {
        ...
        while submitted < total_frames && in_flight.len() < PIPELINE_DEPTH {
            let time = MediaTime::from_ticks(
                ((submitted as f64 / fps) * time::TICKS_PER_SECOND as f64).round() as i64,
            );

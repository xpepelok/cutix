...
use crate::mel::{self, MelExtractor, CHUNK_FRAMES, CHUNK_SAMPLES, N_MELS, SAMPLE_RATE};
...
pub fn remove_whisper_cache(spec: &WhisperSpec) -> Result<(), MlError> {
    let directory = whisper_directory(spec);
    if directory.exists() {
        fs::remove_dir_all(&directory).map_err(|error| MlError::Download(error.to_string()))?;
    }
    Ok(())
}
...
fn argmax_range(logits: &[f32], from: usize, to: usize) -> u32 {
    ...
    for index in from..to.min(logits.len()) {

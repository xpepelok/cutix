...
impl Compositor {
    pub fn share_effect_tables_from(&mut self, other: &EffectPipeline) {
        self.effects.share_tables_from(other);
    }
    ...
    pub fn set_texture_budget(&mut self, budget: usize) {
        self.textures.set_budget(budget);
    }

    pub fn set_render_pool_budget(&mut self, budget: usize) {
        self.texture_pool.set_budget(budget);
    }

    pub fn render_pool_stats(&self) -> (usize, u64, u64, u64) {
        self.texture_pool.stats()
    }

    pub fn texture_stats(&self) -> (usize, usize, u64) {
        (
            self.textures.resident_bytes(),
            self.textures.len(),
            self.textures.evictions(),
        )
    }
    ...
    pub fn render_frame(
        ...
        let scene = self.encode_frame(context, &mut encoder, frame)?;
    ...
    pub fn render_frame_to_texture(
        ...
        let scene = self.encode_frame(context, &mut encoder, frame)?;
        let output =
            context.create_render_texture(frame.width, frame.height, "compositor-offscreen-output");
        let view = output.create_view(&wgpu::TextureViewDescriptor::default());
        context.encode_texture_blit_to_view(
            &mut encoder,
            &scene,
            &view,
            "compositor-offscreen-pass",
        );
        context.queue().submit([encoder.finish()]);
        Ok(output)
    }

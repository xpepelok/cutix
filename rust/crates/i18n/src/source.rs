...
mod tests {
    ...
    fn default_urls_point_at_the_fork() {
        std::env::remove_var("CUTIX_LANG_URL");
        std::env::remove_var("CUTIX_LANG_BRANCH");
        assert_eq!(
            manifest_url(),
        ...
        std::env::set_var("CUTIX_LANG_BRANCH", "ru-localization");
        assert!(manifest_url().contains("/cutix/ru-localization/lang/"));

        std::env::set_var("CUTIX_LANG_URL", "http://127.0.0.1:8321/lang/");

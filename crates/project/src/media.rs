...
use crate::store::{ProjectStore, write_atomic};
...
impl MediaStore {
    ...
    pub fn source_file(&self, asset: &MediaAssetData) -> PathBuf {
        if let Some(linked) = asset.source_path.as_deref() {
            let linked = PathBuf::from(linked);
            if linked.is_file() {
                return linked;
            }
        }
        self.local_file(asset)
    }

    pub fn local_file(&self, asset: &MediaAssetData) -> PathBuf {
    ...
    pub fn remove(&self, media_id: &str) -> Result<()> {
        ...
        let source = self.local_file(&asset);
        if source.is_file() {
    ...
    pub fn import(&self, source: &Path) -> Result<MediaAssetData> {
        ...
        let mut asset = MediaAssetData {
            ...
            source_path: Some(linked),
        };

        if let Ok(Some(path)) = self.generate_thumbnail(&media_id, source, &probe_result) {

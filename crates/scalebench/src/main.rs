...
fn sizes() {
    ...
    for elements in [50usize, 200, 500, 1000, 2000, 5000] {
        let shape = generate::Shape {
            elements,
            media_files: (elements / 5).clamp(10, 200),
            minutes: 30.0,
            ..generate::Shape::default()
        };

        let start = Instant::now();
        let project = generate::build(&shape, 0x5EED);
...
fn library() {
    ...
    for count in [1usize, 5, 10, 25, 50] {
        ...
        for index in 0..count {
            let project = generate::build(&shape, 0x5EED + index as u64);
            store.save(&project).expect("save");
...
fn edit_churn(cycles: u64) {
    ...
    for cycle in 0..cycles {
        let mut project = generate::build(&shape, 0x5EED + cycle);
        store.save(&project).expect("save");
    ...
    println!(
        "  delta over {cycles} open/edit/save/close cycles: working_set {:+.1} MB, private {:+.1} MB, peak {:.1} MB",
        mem::mb(end.working_set) - mem::mb(baseline.working_set),
        mem::mb(end.private) - mem::mb(baseline.private),
        mem::mb(end.peak_working_set)
    );
    let _ = std::fs::remove_dir_all(&directory);
}
...
fn edits() {
    ...
    for elements in [500usize, 2000, 5000] {
        ...
        let mut project = generate::build(&shape, 0x5EED);

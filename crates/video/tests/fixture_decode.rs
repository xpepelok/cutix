use video::{VideoStream, first_frame, probe};
...
fn the_backend_seam_decodes_a_real_frame() {
    ...
    let frame = video::frame_at(&path, 0.0).expect("frame through the seam");
...
fn routing_through_the_seam_changes_no_pixel() {
    ...
    for seconds in [0.0, 1.0, 2.0] {
        ...
        let seamed = video::frame_at(&path, seconds).expect("seam path");

...
pub const AV_OPT_SEARCH_CHILDREN: c_int = 1;
...
/// FFmpeg's "no timestamp" marker, `INT64_MIN`, used for missing `pts`, `dts` and
/// `start_time` values alike.
...
pub type SwsGetCoefficients = unsafe extern "C" fn(c_int) -> *const c_int;
pub type SwsSetColorspaceDetails = unsafe extern "C" fn(
    *mut c_void,
    *const c_int,
    c_int,
    *const c_int,
...
pub type AvGetPixFmt = unsafe extern "C" fn(*const c_char) -> c_int;
...
pub type AvOptSet = unsafe extern "C" fn(*mut c_void, *const c_char, *const c_char, c_int) -> c_int;
pub type AvOptSetInt = unsafe extern "C" fn(*mut c_void, *const c_char, i64, c_int) -> c_int;

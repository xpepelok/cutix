...
use super::{Api, null_mut, null_void};
...
pub(crate) unsafe fn media_origin(api: &Api, format: *mut AVFormatContext) -> f64 {
    ...
    if format_start != sys::AV_NOPTS_VALUE {
        return media_origin_seconds(format_start, 0.0);
    }
    let index = unsafe {
        (api.av_find_best_stream)(format, sys::AVMEDIA_TYPE_VIDEO, -1, -1, null_mut(), 0)
    };
    let count = unsafe { (*format).nb_streams } as c_int;
    if index < 0 || index >= count {
        return 0.0;
    }
    let stream: *mut AVStream = unsafe { *(*format).streams.add(index as usize) };
    let (start_time, time_base) = unsafe { ((*stream).start_time, (*stream).time_base.as_f64()) };
...
fn frame_seconds(
    ...
    {
        Some(stamp) => stamp as f64 * time_base - start_offset,
        None => previous.map_or(0.0, |previous| previous + nominal_span),
    }
}
...
impl Demuxer {
    fn open(path: &Path) -> Result<Self, DecodeError> {
        ...
        demuxer.start_offset = unsafe { media_origin(api, demuxer.format) };
        ...
        demuxer.nominal_span = if average_rate > 0.0 {
            1.0 / average_rate
        } else {
            1.0 / 30.0
        };
        ...
        super::use_every_core(api, demuxer.codec);
    ...
    fn seek(&mut self, seconds: f64) -> Result<(), DecodeError> {
        let seconds = seconds + self.start_offset;
    ...
    fn frame_timestamp(&mut self) -> f64 {
        ...
        self.previous_timestamp = Some(seconds);
        seconds
    }
    ...
    fn render_rgba(&mut self, override_spec: Option<ColorSpec>) -> Result<Frame, DecodeError> {
        ...
        if self.scale_to_rgba(width, height, format, spec, &mut rgba) {
            return Ok(Frame {
                width,
                height,
                rgba,
        ...
        if format == sys::AV_PIX_FMT_YUV420P {
            let planes = unsafe { self.planes(width, height) };
            color::i420_to_rgba(
                planes.y,
                planes.u,
                planes.v,
    ...
    fn scale_to_rgba(
        ...
        let Some(rgba_format) = rgba_pixel_format(self.api) else {
            return false;
        };
        let matrix = match spec.matrix {
            crate::color::Matrix::Bt601 => sys::SWS_CS_ITU601,
            crate::color::Matrix::Bt709 => sys::SWS_CS_ITU709,
            crate::color::Matrix::Bt2020 => sys::SWS_CS_BT2020,
        };
        let key = ScalerKey {
            ...
            destination_format: rgba_format,
        ...
        if self.scaler.is_null() || self.scaler_key != Some(key) {
            ...
            let coefficients = unsafe { (self.api.sws_getCoefficients)(matrix) };
            ...
            let applied = unsafe {
                (self.api.sws_setColorspaceDetails)(
                    self.scaler,
                    coefficients,
                    c_int::from(spec.full_range),
                    coefficients,
                    1,
                    0,
                    1 << 16,
        ...
        let destination: [*mut u8; 4] = [target.as_mut_ptr(), null_mut(), null_mut(), null_mut()];
        let destination_stride: [c_int; 4] = [(width * 4) as c_int, 0, 0, 0];
...
static RGBA_PIXEL_FORMAT: std::sync::OnceLock<Option<c_int>> = std::sync::OnceLock::new();

fn rgba_pixel_format(api: &Api) -> Option<c_int> {
    *RGBA_PIXEL_FORMAT.get_or_init(|| {
        let name = std::ffi::CString::new("rgba").expect("literal");
        let format = unsafe { (api.av_get_pix_fmt)(name.as_ptr()) };
        (format >= 0).then_some(format)
    })
}

type Step = (f64, Option<Frame>);
...
impl FfmpegStream {
    ...
    fn frame_span(&self) -> f64 {
        let rate = self.demuxer.info.frame_count as f64;
        let seconds = self.demuxer.info.duration_seconds;
        let average = if rate > 1.0 && seconds > 0.0 {
            seconds / rate
        } else {
            0.0
        };
        average.max(self.widest_gap)
    }

    fn next_frame(&mut self, keep_from: Option<f64>) -> Result<Option<Step>, DecodeError> {
        if !self.demuxer.next_decoded()? {
            return Ok(None);
        }
        let timestamp = self.demuxer.frame_timestamp();
        if let Some((previous, _)) = self.held.as_ref().or(self.current.as_ref()) {
            let gap = timestamp - *previous;
            if gap > self.widest_gap && gap.is_finite() {
                self.widest_gap = gap;
            }
        }
        self.decoded_samples += 1;

        let superseded = keep_from.is_some_and(|earliest| timestamp < earliest);
        if superseded {
            return Ok(Some((timestamp, None)));
        }
        let frame = self.demuxer.render_rgba(self.spec)?;
        Ok(Some((timestamp, Some(frame))))
    }
    ...
    pub fn frame_at_with_color(
        ...
        for attempt in 0..2 {
            let keep_from = (attempt == 0).then(|| target - self.frame_span());
            ...
            loop {
                ...
                match self.next_frame(keep_from)? {
            ...
            if let Some((timestamp, Some(frame))) = self.current.as_ref() {
...
mod tests {
    ...
    fn a_transport_stream_start_is_measured_from_its_first_timestamp() {
        ...
        let first = frame_seconds(126_000, 126_000, time_base, offset, None, 0.04);
        assert!(first.abs() < 1e-9, "first frame at {first}");
        assert_eq!(stream_start_seconds(sys::AV_NOPTS_VALUE, time_base), 0.0);
    }
    ...
    fn video_and_audio_share_the_container_origin_and_keep_their_offset() {
        ...
        assert!((video - 0.066_7).abs() < 1e-6, "video starts at {video}");
    }

    #[test]
    fn a_container_without_a_start_falls_back_to_the_given_origin() {
        assert_eq!(media_origin_seconds(sys::AV_NOPTS_VALUE, 1.25), 1.25);
        assert_eq!(media_origin_seconds(0, 1.25), 0.0);
    }

    #[test]
    fn a_frame_without_a_pts_falls_back_to_dts_then_to_the_previous_frame() {
        let time_base = 1.0 / 1_000.0;
        let from_dts = frame_seconds(sys::AV_NOPTS_VALUE, 2_000, time_base, 0.0, None, 0.04);
        assert!((from_dts - 2.0).abs() < 1e-9);
        let filled = frame_seconds(
            sys::AV_NOPTS_VALUE,
            sys::AV_NOPTS_VALUE,
            time_base,
            0.0,
        ...
        assert!(
            (filled - 3.04).abs() < 1e-9,
            "not sent back to zero: {filled}"
        );
    }

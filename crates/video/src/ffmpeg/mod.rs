...
pub use audio::{AudioBuffer, AudioInfo, decode_audio, decode_audio_range, probe_audio};
pub use encode::{AAC_FRAME_SAMPLES, can_encode_aac, encode_aac, frame_channel_layout_offset};
pub use video::{DynamicRange, FfmpegStream, Transfer, dynamic_range, frame_at_with_color, probe};
pub use video_encode::{
    EncodedPacket, H264Encoder, available as available_h264_encoders, best as best_h264_encoder,
    is_hardware as is_hardware_h264,
};
...
pub struct Api {
    ...
    pub sws_getCoefficients: sys::SwsGetCoefficients,
    pub sws_setColorspaceDetails: sys::SwsSetColorspaceDetails,
    pub av_get_pix_fmt: sys::AvGetPixFmt,
    ...
    pub av_opt_set: sys::AvOptSet,
    pub av_opt_set_int: sys::AvOptSetInt,
}
...
unsafe fn bind(
    ...
    unsafe {
        Ok(Api {
            ...
            sws_getCoefficients: symbol(swscale, b"sws_getCoefficients\0")?,
            sws_setColorspaceDetails: symbol(swscale, b"sws_setColorspaceDetails\0")?,
            av_get_pix_fmt: symbol(avutil, b"av_get_pix_fmt\0")?,
            ...
            av_opt_set: symbol(avutil, b"av_opt_set\0")?,
            av_opt_set_int: symbol(avutil, b"av_opt_set_int\0")?,
        })
    }
}
...
pub(crate) fn set_option(api: &Api, target: *mut c_void, name: &str, value: &str) -> bool {
    let (Ok(name), Ok(value)) = (std::ffi::CString::new(name), std::ffi::CString::new(value))
    else {
        return false;
    };
    unsafe {
...
pub(crate) fn set_option_int(api: &Api, target: *mut c_void, name: &str, value: i64) -> bool {
    let Ok(name) = std::ffi::CString::new(name) else {
        return false;
    };
    unsafe { (api.av_opt_set_int)(target, name.as_ptr(), value, sys::AV_OPT_SEARCH_CHILDREN) == 0 }
}

pub(crate) fn use_every_core(api: &Api, codec_context: *mut c_void) {
    set_option_int(api, codec_context, "threads", 0);
}

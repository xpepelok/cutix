...
mod tests {
    ...
    #[test]
    fn a_higher_quality_always_quantises_more_finely_than_a_lower_one() {
        let mut previous: Option<u8> = None;
        for quality in EXPORT_QUALITIES {
            let quantiser = quality.quantiser();
            assert!(quantiser <= 51, "{quality:?} is outside the H.264 range");
            if let Some(coarser) = previous {
                assert!(
                    quantiser < coarser,
                    "{quality:?} quantises at {quantiser}, no finer than the quality below"
                );
            }
            previous = Some(quantiser);
        }
    }

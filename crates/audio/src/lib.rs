...
pub use denoise::{DenoiseOptions, denoise, estimate_noise_profile, rms};
pub use silence::{
    DEFAULT_MIN_SILENCE_SECONDS, DEFAULT_SILENCE_PADDING_SECONDS, DEFAULT_SILENCE_THRESHOLD_DB,
    SilenceOptions, SilentRange, detect_silent_ranges,
};
...
mod tests {
    ...
    fn speech_burst(sample_rate: u32, seconds: f32, start: f32, end: f32) -> Vec<f32> {
        ...
        for (offset, sample) in samples[from..to].iter_mut().enumerate() {
